...
async def process_reaction(reaction, user, client):
    found = False
    if reaction.message.id in confirmation_requests and user != client.user:
        if(user == confirmation_requests[reaction.message.id].member or not confirmation_requests[reaction.message.id].member):
            found = True
            if reaction.emoji == '✅':
